...
df_laptime = spark.read.csv('s3://columbia-gr5069-main/raw/lap_times.csv', header=True)
...
df_lap_drivers = df_driver.join(df_laptime, on=['driverId'])
...
df_lap_drivers = df_driver.select('driverId', 'nationality', 'age', 'forename', 'surname', 'url').join(df_laptime, on=['driverId'])
...
df_lap_drivers.write.csv('s3://zd23260-gr5069/processed/in_class_workshop_2024/laptimes_by_drivers.csv')
